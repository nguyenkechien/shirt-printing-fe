...
class Mockup {
  constructor(color, display, template, callback) {
  ...
  refreshImage(img) {
    ...
    let clonedImage = self.captureLayer.findOne((node) => {
      return (
  ...
  refreshText(txt) {
    ...
    let clonedText = self.captureLayer.findOne((node) => {
      return (
  ...
  convertCanvasToSVG(canvas, fileName) {
    if (!canvas) return;
